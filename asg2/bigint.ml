...

    ...
    let remainder, product = mul' list1 (two_times p2) (two_times list2)  in
        if((cmp' remainder p2) < 0 ) then remainder, product
        else (sub' remainder p2 0), (add' product list2 0)

    let mul (Bigint (neg1, value1)) (Bigint (neg2, value2)) =
    if neg1 = neg2 
        then let product = mul' value1 [1] value2 in Bigint(neg1,product )
    else 
        let product = mul' value1 [1] value2 in Bigint(Neg,product )
